...
def bot_background_loop():
    ...
    while True:
        try:
            ...
            # [TIMING OPTIMIZATION]: Allow entry from T-5m (300s) down to T-5s so no funding interval is ever missed
            is_entry_window    = 5 <= secs <= 300
    ...
    <script>
        ...
        async function updateDashboard() {
            try {
                ...
                const dBal = data.state.delta_balance ?? 0.00;
                const cBal = data.state.coindcx_balance ?? 0.00;
