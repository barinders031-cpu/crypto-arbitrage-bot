"""
download_real_market_data.py
============================
Robust Historical Data Downloader for ML Model Testing.
Downloads Spot and Options (CE & PE) data for major indices.

Features:
- Rate Limit Protection (exponential backoff)
- Batching & Error Resilience
...
OUTPUT_DIR        = "live_data" # Folder to save data

# Master list of Indices to process
INDICES = {
    "NIFTY":     {"exch": "NSE", "token": "99926000", "gap": 50},
    "BANKNIFTY": {"exch": "NSE", "token": "99926009", "gap": 100},
        ...
        while retries > 0:
            ...
            if res and res.get("status") and res.get("data"):
                ...
                if "limit" in msg or "too many" in msg or "access" in msg:
                    ...
                    retries -= 1
                    continue
            # Break if it's an unrecoverable error or empty data for that period
            break
            
        current_start = current_end + timedelta(minutes=1)
        time.sleep(1) # Safe pause between chunk requests
    ...
    while retries > 0:
        ...
        if res and res.get("status") and res.get("data"):
            ...
            if "limit" in msg or "too many requests" in msg:
                ...
                retries -= 1
                continue
        break # Return empty if hard failure or no data
    return []
...
def run():
    print("=" * 60)
    print("  REAL MARKET DATA DOWNLOADER (ML TRAINING READY)")
    print("=" * 60)
    ...
    for idx_name, info in INDICES.items():
        ...
        # 1. Fetch Spot
        spot_file = os.path.join(OUTPUT_DIR, f"{idx_name}_spot_{HISTORY_DAYS}d.csv")
        ...
        # 3. Download Options
        opt_file = os.path.join(OUTPUT_DIR, f"{idx_name}_options_{HISTORY_DAYS}d.csv")
        success = 0
        total_rows = 0
        ...
        with open(opt_file, 'w', newline='') as f:
            ...
            for i, tok_info in enumerate(opt_tokens):
                ...
                time.sleep(0.3) # Hard delay to prevent triggering Angel's strict limits
